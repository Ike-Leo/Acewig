...
All HTTP endpoints are prefixed with the following base URL structure:

```
https://<YOUR_CONVEX_DEPLOYMENT_URL>.convex.site/api/store/<ORG_SLUG>
```

⚠️ **Important:** Use `.convex.site` for HTTP routes, NOT `.convex.cloud`.

*   `YOUR_CONVEX_DEPLOYMENT_URL`: Your specific project instance (e.g., `acoustic-seahorse-440`).
*   `ORG_SLUG`: The unique identifier for the specific store/organization you are interacting with (e.g., `hair-palace`, `my-coffee-shop`).

**Example:** 
```
https://acoustic-seahorse-440.convex.site/api/store/hair-palace/products
```
